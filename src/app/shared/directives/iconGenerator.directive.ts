...
export function centerjs(options) {
  /**
   * Set defaults.
   */
  const defaults = {
...
@Directive({
    selector: '[iconGenerator]'
}) 

export class IconGeneratorDirective implements OnInit {
	@Input() txt:string
    ...
    abbreviate = (str) => {
    	let postStr
    	let arr = str.split(' ')
    	let arr2 = arr.map(x => x.substring(0,1))
    	postStr = arr2.join('')
    	return postStr
    }
